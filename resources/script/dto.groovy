...
def generate(table, dir) {
    ...
    def dtoBaseName = className + "DTOBase"
    def dtoName = className + "DTO"
    def dtoBaseFile = new File(dir + "/dto/base")
    dtoBaseFile.mkdirs()
    new File(dtoBaseFile.getPath(), dtoBaseName+".java").withPrintWriter { out ->
        printOut = out
        generateFile(className, fields)
    }
    new File(dtoBaseFile.getParent(),dtoName+".java").withPrintWriter {out ->
        printOut = out
        generateDTO(dtoName,dtoBaseName)
    }
}

def generateDTO(dtoName , dtoBaseName){
    printlnWithTab("package " + packageName + ".dto;")
    printlnWithTab("")
    printlnWithTab("import "+packageName+".dto.base.$dtoBaseName;")
    printlnWithTab("")
    printlnWithTab("public class $dtoName extends $dtoBaseName { ")
    printlnWithTab("")
...
def generateFile(className, fields) {
    printlnWithTab("package "+packageName+".dto.base;")
    printlnWithTab("")
    if(importList != null) importList.each{path -> printlnWithTab("import " + path + ";")}
    printlnWithTab("")
    printlnWithTab("public abstract class "+className+"DTOBase implements Serializable {")
    printlnWithTab("")
    printlnWithTab("private static final long serialVersionUID = 1L;" , 1)
    printlnWithTab("")
    fields.each() {
